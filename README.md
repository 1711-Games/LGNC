...
any time, not to mention that you have to map every request/response to a struct/class, and sometimes response
...
contracts for BE, guarantee them and just hook up business logic to it. You will never catch an "oof, I expected
this field to be integer, and it actually is, but for some reason in quotes, and because of that Codable failed
...
gentlemen's agreements), whereas microservice arch is a quite reasonable approach for many cases (not all).
...
those contracts as if it's a module in your service, and not a dedicated service in a different language on
a remote machine.
...
(as well as Linux) can do now. [↩](#a1)</sub>
